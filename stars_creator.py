...
for h in range(height):
    for w in range(width):
        ...
        if(matrix_pos in opacity_stars_array):
            ...
            categories_list["opacity"].append(f'<div class="star dynamic_opacity_star star_squad_{star_squad}" style="left:calc({math.floor(w*(105/width))}% + {rannumX}px);top:calc({math.floor(h*(105/height))}% + {rannumY}px);"></div>')
        else:
            categories_list[f"squad_{star_squad}"].append(f'<div class="star static_opacity_star star_squad_{star_squad}" style="left:calc({math.floor(w*(105/width))}% + {rannumX}px);top:calc({math.floor(h*(105/height))}% + {rannumY}px);"></div>')
            #file_content+=
